...
const validateEmail = (email) => {
  var isemail=/^[a-z0-9]([a-z0-9])*(.[a-z0-9])([a-z0-9]*)@[a-z0-9]([a-z0-9]*).(com|net|org)$/gmi;
...
const validatePhoneNumber = (phoneNumber) => {
  const isphonenumber = /^(\(\d{3}\)|\d{3})(-| )?\d{3}(-| )?\d{4}$/gm;
...
const findTagNames = elements => {
  var istag = /<\/([a-z0-9])*>/g;
  var unparsed = elements.map( str => {
    return str.match(istag);
  });
  var tags = [];
  unparsed = unparsed.map(array => {
    array.forEach(tag => {
      tag = tag.split('');
